...
contract SavePlanetEarth is Context, IERC20, Ownable {
    ...
    uint256 private _totalReflection = (~uint256(0) -
        (~uint256(0) % _totalSupply));
    ...
    function setMaxTxPercent(uint256 maxTxPercent) external onlyOwner() {
        _maxTaxAmount = _totalSupply.mul(maxTxPercent).div(10**2);
    }
    ...
    function _getCurrentSupply() private view returns (uint256, uint256) {
        ...
        for (uint256 i = 0; i < _excluded.length; i++) {
            if (
                ...
                _totalOwned[_excluded[i]] > tSupply
            ) return (_totalReflection, _totalSupply);
            rSupply = rSupply.sub(_reflectOwned[_excluded[i]]);
            tSupply = tSupply.sub(_totalOwned[_excluded[i]]);
        }
        if (rSupply < _totalReflection.div(_totalSupply))
            return (_totalReflection, _totalSupply);
    ...
    function _takeLiquidity(uint256 tLiquidity) private {
        ...
        if (_isExcluded[address(this)])
            _totalOwned[address(this)] = _totalOwned[address(this)].add(
                tLiquidity
            );
    }

    function calculateTaxFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_transTaxReflectFee).div(10**2);
    }
    ...
    function calculateLiquidityFee(uint256 _amount)
        ...
        returns (uint256)
    {
        return _amount.mul(_transTaxliquidityFee).div(10**2);
    }
